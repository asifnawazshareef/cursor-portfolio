...
let inMemoryPortfolio: PortfolioData = normalizePortfolio(portfolioData);
...
export async function getPortfolioData(): Promise<PortfolioData> {
  try {
    ...
    inMemoryPortfolio = normalizePortfolio(parsed);
    return inMemoryPortfolio;
  } catch {
    // Fallback for environments with read-only or restricted filesystem.
    return inMemoryPortfolio;
  }
}
...
export async function updatePortfolioData(payload: PortfolioData) {
  ...
  inMemoryPortfolio = normalized;

  try {
    await ensureDataFile();
    await fs.writeFile(DATA_FILE, JSON.stringify(normalized, null, 2), "utf8");
  } catch {
    // Keep app functional even if persistence is unavailable.
  }
